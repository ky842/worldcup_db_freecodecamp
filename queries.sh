...
echo "$($PSQL "(SELECT DISTINCT(name) FROM teams JOIN games AS win ON teams.team_id = win.winner_id WHERE win.round='Eighth-Final' AND year = 2014
               UNION SELECT DISTINCT(name) FROM teams JOIN games AS lose ON teams.team_id = lose.opponent_id WHERE lose.round='Eighth-Final' AND year = 2014) ORDER BY name
")"
